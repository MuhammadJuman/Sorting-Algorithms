package sortingAlgorithmsPkg;
import java.util.Random;

/**
 * SortingAlgorithms.java
 * A class representing the GUI used in sorting algorithms.
 * <pre>
 * Hours:        4 hours
...
public class SortingAlgorithms extends javax.swing.JFrame {
    ...
    private final int MAX_NUMBER = 15; // Number of integers to be generated
    ...
    {
        initComponents();
        this.getRootPane().setDefaultButton(createJButton); //set buttonAdd as default
	...
	for (int k = low; k < high; k++)
    {
		...
		else if (j == high) temp[k] = array[i++]; //if mid-hi array is empty
		else if (array[j] > array[i]) temp[k] = array[i++]; //i is lower so we put it in the array first
		else temp[k] = array[j++]; //j is lower or equal to i, so we put it in the array first.
	}
		//Now we need to copy back temp array to its place in A array
	for (int k = low; k < high; k++)
		array[k] = temp[k]; //we are copying only the numbers we just merged.
    }
	...
	if (l < heapSize) // check if left child is an existing child
        ...
        { 
            ...
            if(array[r] > array[max])// if right child is bigger than our current maximum
            { 
